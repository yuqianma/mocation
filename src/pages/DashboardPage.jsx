...
function formatSelectedTimestamp(value) {
  if (!value) {
    return '--';
  }
...
export default function DashboardPage() {
  ...
  const { fetchPointsForCurrentRange, setBlur, hidePicker, range, selectedPointTimestamp } = useApp();

  const fromLabel = formatBoundary(range[0]);
  const toLabel = formatBoundary(range[1]);
  const selectedLabel = formatSelectedTimestamp(selectedPointTimestamp);
    ...
    <div className="pointer-events-none relative h-full w-full">
      <div className="absolute left-4 top-4 z-20 sm:left-6 sm:top-6">
        <div className="rounded-xl border border-white/55 bg-white/35 px-4 py-2 text-xs text-slate-700 shadow-lg backdrop-blur-sm sm:px-5">
          ...
          <div className="mt-2 border-t border-white/45 pt-1.5 text-[11px] text-slate-700/95">
            <span className="font-medium">Selected:</span> <span>{selectedLabel}</span>
          </div>
        </div>
      </div>
      <QueryPanel />
